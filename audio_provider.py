...
def huggingface_tts(text: str, output_path: Optional[str] = None, **kwargs) -> Optional[str]:
    ...
    if hasattr(audio, "read"):
        data = audio.read()
        if data:
            return _save_bytes(data, output)
    raise RuntimeError(f"Unsupported Hugging Face TTS response: {type(audio).__name__}")
...
def fal_music(prompt: str, output_path: Optional[str] = None, **kwargs) -> Optional[str]:
    ...
    model = os.getenv("FAL_MUSIC_MODEL") or "fal-ai/stable-audio-3/small/music/text-to-audio"
    output = Path(output_path or OUTPUT_DIR / "music-fal.mp3")
    result = fal_client.subscribe(
        model,
        arguments={"prompt": prompt, "duration": kwargs.get("duration", 8), "output_format": kwargs.get("output_format", "mp3")},
        with_logs=False,
    )
    if not result:
        raise RuntimeError("FAL Music returned no result")
...
def huggingface_musicgen(prompt: str, output_path: Optional[str] = None, **kwargs) -> Optional[str]:
    ...
    model = os.getenv("HF_MUSIC_MODEL", "m-a-p/YuE-s1-7B-anneal-en-cot")
    output = Path(output_path or OUTPUT_DIR / "music-huggingface.mp3")
    client = InferenceClient(provider=provider, api_key=token, timeout=600)
    genres = kwargs.get("genres", "professional cinematic technology background music")
    audio = client.text_to_speech(prompt, model=model, extra_body={"genres": genres})
    if isinstance(audio, bytes):
        return _save_bytes(audio, output)
    if isinstance(audio, bytearray):
        return _save_bytes(bytes(audio), output)
    if hasattr(audio, "read"):
        ...
        if data:
            return _save_bytes(data, output)
    raise RuntimeError(f"Unsupported Hugging Face Music response: {type(audio).__name__}")
